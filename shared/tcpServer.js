    ...
    socket.on('data', async (chunk) => {
        ...
        while (cursor < socket.sessionBuffer.length) {
            ...
            if (isTextReply && socket.imei) {
                logger.debug(`[TCP] ⏳ Delaying ACK for text reply: ${ackBuf.toString('hex').toUpperCase()}`);
                ...
                socket.ackTimeout = setTimeout(() => {
                    if (socket.pendingAck && socket.writable) {
                        ...
                        logger.debug(`[TCP] 📤 Sent Delayed ACK to ${socket.imei}: ${socket.pendingAck.toString('hex').toUpperCase()}`);
                ...
                if (socket.writable) {
                    socket.write(ackBuf);
                    logger.debug(`[TCP] 📤 Sent ACK: ${ackBuf.toString('hex').toUpperCase()}`);
                } else {
